...
progenyConstructor = function(mode, settings) {
  var addDep, altPaths, exclusion, extension, extensionsList, multipass, parseDeps, potentialDeps, prefix, progeny, progenySync, regexp, reverseArgs, rootPath;
  if (settings == null) {
    settings = {};
  }
  ...
  parseDeps = function(path, source, depsList, callback) {
    ...
    paths = source.toString().split('\n').map(function(line) {
      ...
      if (extension && !glob.hasMagic(path) && '' === sysPath.extname(path)) {
    ...
    if (extension) {
      deps.forEach(function(path) {
        if (!(("." + extension) === sysPath.extname(path) || glob.hasMagic(path))) {
      ...
      return each(deps, function(path, callback) {
        ...
        if (__indexOf.call(depsList, path) >= 0) {
          ...
          if (glob.hasMagic(path)) {
            ...
            return fs.exists(modulePath, function(exists) {
              if (exists) {
                depsList.splice(depsList.indexOf(path), 1, modulePath);
                return addDep(modulePath, depsList, callback);
              } else {
                return addDep(path, depsList, callback);
              }
            });
  ...
  addDep = function(path, depsList, callback) {
