...
progenyConstructor = function(mode, settings) {
  ...
  parseDeps = function(path, source, depsList, callback) {
    ...
    paths = source.toString().split('\n').map(function(line) {
      ...
      var allowExtendedImports;
      allowExtendedImports = globDeps && glob.hasMagic(path) || moduleDep;
    ...
    dirs.forEach(function(dir) {
      return paths.forEach(function(path) {
        if (moduleDep && extension && '' === sysPath.extname(path)) {
          ...
          return deps.push(sysPath.join(dir, path, "index." + extension));
        } else {
    ...
    if (extension) {
      deps.forEach(function(path) {
        var isGlob;
        isGlob = globDeps && glob.hasMagic(path);
        if (!isGlob && ("." + extension) !== sysPath.extname(path)) {
